git --version
https://github.com/baileyhallwachs/cereo-r-S2021.git
library(githubinstall)
git clone https://github.com/git/git

# explore the bike data to look at the relationship 
#between temperature and number of riders. 
#
# Step: Load the bike data and look at the metadata 
library(tidyverse)
bike <- read.csv("daily_bike_data.csv")

head(bike)
str(bike)
sort(names(bike))

#Time trend of ridership (seasonal/time trend )
...
rides %>%
  ...
  pivot_longer(names_to = "month", values_to = "rides",
               cols = c("rides_january", "rides_february")) %>%
  mutate(month = substr(month, 7, nchar(month))) %>%
  mutate(month = toupper(month))

#Examples of equivalent commands in gather vs pivot_long and spread vs pivot_wider
pivot_wider(data = bike, names_from = mnth, values_from = cnt)
spread(data = bike, key = mnth, value = cnt)

pivot_longer(data = bike, names_to = "day_type", values_to = "count", cols = c("holiday", "weekday", "workingday"))
gather(data = bike, key = "day_type", value = "count",  c("holiday", "weekday", "workingday"))
#Note that key & value in gather/spread are replaced by names_to/from and values_to/from in the pivot* functions
...
# Saving data 
write_csv(bike3, "bike_final.csv")
